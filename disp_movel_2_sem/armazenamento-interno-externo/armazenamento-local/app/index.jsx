...
const audioSource = [
  ...
  require('../assets/sounds/song3.mp3'),
];
...
export default function App() {
  const player = [
    useAudioPlayer(audioSource[0]),
    useAudioPlayer(audioSource[1]),
    useAudioPlayer(audioSource[2])
  ];
  ...
  const handleAddTask = () => {
    ...
    const newTasks = [...tasks, { 'task': task, 'check': false }];
    setTasks(newTasks);
        ...
        renderItem={({ item, index }) => (
          ...
          <View style={styles.taskContainer}>
            <Checkbox
              ...
              value={item.check}
              onValueChange={() => chackBox(index)}
              color={item.check ? '#4630EB' : undefined}
            />
